# code to generate Tax profile for 2018 and 2019
# Shreyank Shetty
...
# define function to get best Buy order for BTC trade
def findBTCHighestBuy(df2, index):
    highestTradePrice = 0
    highestBuyIndex = 0
    for i, r in df2.iterrows():
        # check that trade is a buy and weather its already matched or not
        if (r['Txn'] == 'BUY') and (r['isMatch'] == False):
            # check that the trade happened after the BTC trade and also if the  trade price is higest
            if (r['Quantity'] > 0):
                if (i < index) and (r['TradePrice'] > highestTradePrice):
                    highestTradePrice = r['TradePrice']
                    highestBuyIndex = i
    # return the index of higest buy order after the BTC happened
...
def btcMatchAlgo(df2, index, quantity, btcShortTermGain):
    if quantity == 0:
        return btcShortTermGain
    else:
        btcHighestBuyIndex = findBTCHighestBuy(df2, index)
        if btcHighestBuyIndex == 0:
            quantity = 0
            return btcMatchAlgo(df2, index, quantity, btcShortTermGain)

        # check if buy quanity is less that sell order
        if (quantity > df2.loc[btcHighestBuyIndex, 'Quantity']):
            # calculate the gains/losses from trade.
            sGain = ((df2.loc[index, 'TradePrice'])*(df2.loc[btcHighestBuyIndex, 'Quantity']) - (
                df2.loc[btcHighestBuyIndex, 'TradePrice'])*(df2.loc[btcHighestBuyIndex, 'Quantity']))
            # cumulative add the short term gains/losses
            btcShortTermGain = btcShortTermGain + sGain
            # reduce quantity by the quantity of trade matched
            quantity = quantity - (df2.loc[btcHighestBuyIndex, 'Quantity'])
            # update dataframe to mark buy trade as matched
            df2.at[btcHighestBuyIndex, 'isMatch'] = True
            df.at[btcHighestBuyIndex, 'isMatch'] = True
        # if buy order quantity is more than sell quantity
        else:
            # calculate short term gain for the trade matched
            btcShortTermGain = btcShortTermGain + (((df2.loc[index, 'TradePrice'])*quantity) - (
                (df2.loc[btcHighestBuyIndex, 'TradePrice'])*quantity))
            # update buy order quantity. reduce by number of quantity matched
            df2.at[btcHighestBuyIndex, 'Quantity'] = df2.loc[btcHighestBuyIndex,
                                                             'Quantity'] - quantity
            df.at[btcHighestBuyIndex, 'Quantity'] = df.loc[btcHighestBuyIndex,
                                                           'Quantity'] - quantity
            # update sell order as Matched
            df2.at[index, 'isMatch'] = True
            df.at[index, 'isMatch'] = True
...
def calculateTaxProfile(symbol):
    ...
    if ('BTC' in df2.Txn.values) or ('SEL' in df2.Txn.values):
        ...
        for index, row in df2.iterrows():
            ...
            # calculate tax for BTC trades
            if (row['Txn'] == 'BTC'):
                if row['TradeDate'] < date2019:
                    # print(index)
                    quantity = row['Quantity']
                    stockShortGains = btcMatchAlgo(
                        df2, index, quantity, totalShortTermGains2018)
                    totalShortTermGains2018 = stockShortGains
                else:
                    quantity = row['Quantity']
                    gains = btcMatchAlgo(
                        df2, index, quantity, totalShortTermGains2019)
                    totalShortTermGains2019 = gains
